...
import { apiSendTestWebhook, apiTestHomeAssistant, apiBroadcastTts, apiGetCloudBackupConfig, apiSaveCloudBackupConfig, apiGetCloudBackupStatus, apiTriggerCloudBackup, apiGenerateEncryptionKey, apiTestCloudBackupConnection, CloudBackupConfig, CloudBackupStatus, CloudBackupTestResult, apiGetSmartThingsBackupConfig, apiSaveSmartThingsBackupConfig, apiValidateSmartThingsPat, apiTriggerSmartThingsBackup, apiListSmartThingsBackups, apiGetSmartThingsBackup, apiGetLatestSmartThingsBackup, SmartThingsBackupConfig, SmartThingsBackupInfo, SmartThingsPatValidation } from '../services/api';
...
const CloudBackupSettings = () => {
    ...
    const [testResult, setTestResult] = useState<CloudBackupTestResult | null>(null);

    // Local form state
    const [localConfig, setLocalConfig] = useState<CloudBackupConfig>({
        enabled: false,
        provider: null,
        schedule: { frequency: 'daily', timeOfDay: '03:00' },
        keepLocalBackups: false,
        notifyOnSuccess: false,
        notifyOnFailure: true,
        s3: { bucket: '', region: 'us-east-1', prefix: 'hometile-backups', retentionDays: 30 },
        gdrive: { folderId: '', retentionDays: 30 }
    });
    ...
    const scheduleFrequencies = [
        ...
        { value: 'tuesday', label: 'Weekly - Tuesday' },
        { value: 'wednesday', label: 'Weekly - Wednesday' },
        { value: 'thursday', label: 'Weekly - Thursday' },
        { value: 'friday', label: 'Weekly - Friday' },
        { value: 'saturday', label: 'Weekly - Saturday' },
    ];
        ...
        const fetchData = async () => {
            try {
                const [configData, statusData] = await Promise.all([
                    apiGetCloudBackupConfig(),
                    apiGetCloudBackupStatus()
                ]);
                ...
                if (configData) {
                    setLocalConfig(prev => ({
                        ...prev,
                        ...configData,
                        schedule: configData.schedule || prev.schedule,
                        s3: { ...prev.s3, ...configData.s3 },
                        gdrive: { ...prev.gdrive, ...configData.gdrive }
                    }));
                }
            } catch (err) {
                console.error('Failed to fetch cloud backup config:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    useEffect(() => {
        const fetchStatus = async () => {
            try {
                const statusData = await apiGetCloudBackupStatus();
                setStatus(statusData);
            } catch (err) {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await apiSaveCloudBackupConfig(localConfig);
            setConfig(localConfig);
            addNotification('Cloud backup configuration saved!', 'success');
        } catch (err: any) {
    ...
    const handleBackupNow = async () => {
        setIsBackingUp(true);
        try {
            const result = await apiTriggerCloudBackup();
            if (result.success) {
                addNotification(`Backup completed: ${result.filename}`, 'success');
                // Refresh status
                const statusData = await apiGetCloudBackupStatus();
                setStatus(statusData);
            } else {
            ...
            addNotification(`Backup error: ${err.message}`, 'error');
        } finally {
    ...
    const handleGenerateKey = async () => {
        ...
        try {
            const result = await apiGenerateEncryptionKey();
            setLocalConfig(prev => ({ ...prev, encryptionKey: result.key }));
            addNotification('Encryption key generated. Save configuration to apply.', 'success');
        } catch (err: any) {
            addNotification(`Failed to generate key: ${err.message}`, 'error');
        } finally {
    ...
    const handleTestConnection = async () => {
        ...
        setTestResult(null);
        try {
            const result = await apiTestCloudBackupConnection(localConfig);
            setTestResult(result);
            if (result.success) {
                addNotification('Connection test passed!', 'success');
            } else {
                addNotification('Connection test failed. See details below.', 'error');
            }
        } catch (err: any) {
            setTestResult({
                success: false,
                tests: [{ name: 'Request', passed: false, error: err.message }]
            });
            addNotification(`Test failed: ${err.message}`, 'error');
        } finally {
            setIsTesting(false);
        }
    };

    const updateLocalConfig = (updates: Partial<CloudBackupConfig>) => {
        setLocalConfig(prev => ({ ...prev, ...updates }));
    };

    const updateS3Config = (updates: Partial<NonNullable<CloudBackupConfig['s3']>>) => {
        setLocalConfig(prev => ({
            ...prev,
            s3: { ...prev.s3!, ...updates }
        }));
    ...
    const updateGDriveConfig = (updates: Partial<NonNullable<CloudBackupConfig['gdrive']>>) => {
        setLocalConfig(prev => ({
            ...prev,
            gdrive: { ...prev.gdrive!, ...updates }
        }));
    };

    const updateSchedule = (updates: Partial<NonNullable<CloudBackupConfig['schedule']>>) => {
        setLocalConfig(prev => ({
            ...prev,
    ...
    if (isLoading) {
        return <div className="p-8 text-center text-gray-400">Loading backup configuration...</div>;
    }
    ...
    return (
        <>
            <AdminSection
                title="Cloud Backup"
                description="Automatically backup your encrypted database to cloud storage for disaster recovery."
            >
                <div className="space-y-6">
                    {/* Enable/Disable */}
                    <AdminToggle
                        label="Enable Automated Cloud Backups"
                        description="Automatically encrypt and upload database backups on a schedule"
                        enabled={localConfig.enabled}
                        onToggle={() => updateLocalConfig({ enabled: !localConfig.enabled })}
                    />
                        ...
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className={`font-semibold ${status.serviceAvailable !== false ? 'text-green-400' : 'text-red-400'}`}>
                                        {status.serviceAvailable !== false ? 'Running' : 'Not Available'}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-400">Provider:</span>
                                    <span className="text-white">{status.provider || 'Not configured'}</span>
                                </div>
                                {status.nextScheduledBackup && (
                                    <div className="flex justify-between">
                                        <span className="text-gray-400">Next Backup:</span>
                                        <span className="text-white">{new Date(status.nextScheduledBackup).toLocaleString()}</span>
                                    </div>
                                )}
                                {status.lastBackupTime && (
                                    <div className="flex justify-between">
                                        <span className="text-gray-400">Last Backup:</span>
                                        <span className="text-white">{new Date(status.lastBackupTime).toLocaleString()}</span>
                                    </div>
                                )}
                                {status.lastBackupResult && (
                                    <div className="flex justify-between">
                                        <span className="text-gray-400">Last Result:</span>
                                        <span className={`font-semibold ${status.lastBackupResult.success ? 'text-green-400' : 'text-red-400'}`}>
                                            {status.lastBackupResult.success ? 'Success' : 'Failed'}
                                        </span>
                                    </div>
                                )}
                                {status.lastBackupResult?.error && (
                                    <div className="mt-2 p-2 bg-red-900/20 border border-red-700 rounded text-sm text-red-300">
                                        Error: {status.lastBackupResult.error}
                                    </div>
                                )}
                            </div>
                            <div className="mt-4">
                                <AdminButton
                                    ...
                                    disabled={isBackingUp || !localConfig.enabled || !localConfig.encryptionKey}
                                >
                    ...
                    {/* Encryption Key */}
                    <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                        <h4 className="font-semibold text-white mb-4">Encryption</h4>
                        <p className="text-sm text-gray-400 mb-4">
                            All backups are encrypted with AES-256-GCM before upload. Store your encryption key securely - you'll need it to restore backups.
                        </p>
                        <div className="space-y-4">
                            <div>
                                <label className="block mb-1 text-sm font-medium text-gray-400">Encryption Key</label>
                                <div className="flex gap-2">
                                    <input
                                        type={showSecrets ? "text" : "password"}
                                        value={localConfig.encryptionKey || ''}
                                        onChange={(e) => updateLocalConfig({ encryptionKey: e.target.value })}
                                        placeholder="Enter or generate an encryption key"
                                        className="flex-1 bg-gray-700 border border-gray-600 rounded-md p-2 text-white focus:ring-brand-blue focus:border-brand-blue font-mono text-sm"
                                    />
                                    <AdminButton
                                        variant="secondary"
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <label htmlFor="show-secrets" className="text-sm text-gray-400">Show sensitive values</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                        <h4 className="font-semibold text-white mb-4">Storage Provider</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <button
                                onClick={() => updateLocalConfig({ provider: 's3' })}
                                className={`p-4 rounded-lg border-2 transition-colors ${localConfig.provider === 's3' ? 'border-brand-blue bg-brand-blue/10' : 'border-gray-600 hover:border-gray-500'}`}
                            >
                                <div className="flex items-center gap-3">
                                    <IconCloud className="w-8 h-8 text-orange-400" />
                                    <div className="text-left">
                                        <div className="font-semibold text-white">Amazon S3</div>
                                        <div className="text-sm text-gray-400">AWS cloud storage</div>
                                    </div>
                                </div>
                            </button>
                            <button
                                onClick={() => updateLocalConfig({ provider: 'gdrive' })}
                                className={`p-4 rounded-lg border-2 transition-colors ${localConfig.provider === 'gdrive' ? 'border-brand-blue bg-brand-blue/10' : 'border-gray-600 hover:border-gray-500'}`}
                            >
                                <div className="flex items-center gap-3">
                                    <IconCloud className="w-8 h-8 text-blue-400" />
                                    <div className="text-left">
                                        <div className="font-semibold text-white">Google Drive</div>
                                        <div className="text-sm text-gray-400">Google cloud storage</div>
                                    </div>
                                </div>
                            </button>
                        </div>
                    ...
                    {/* S3 Configuration */}
                    {localConfig.provider === 's3' && (
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                            <h4 className="font-semibold text-white mb-4">Amazon S3 Configuration</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <AdminInput
                                    label="Bucket Name"
                                    id="s3-bucket"
                                    ...
                                    value={localConfig.s3?.bucket || ''}
                                    onChange={(e) => updateS3Config({ bucket: e.target.value })}
                                />
                                <AdminSelect
                                    label="Region"
                                    id="s3-region"
                                    value={localConfig.s3?.region || 'us-east-1'}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <AdminInput
                                    ...
                                    type={showSecrets ? "text" : "password"}
                                    placeholder="AKIA..."
                                    value={localConfig.s3?.accessKeyId || ''}
                                ...
                                <AdminInput
                                    ...
                                    value={localConfig.s3?.secretAccessKey || ''}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <AdminInput
                                    ...
                                    id="s3-prefix"
                                    type="text"
                                    placeholder="hometile-backups"
                                    value={localConfig.s3?.prefix || ''}
                                    onChange={(e) => updateS3Config({ prefix: e.target.value })}
                                />
                                <AdminInput
                                    ...
                                    min="1"
                                    max="365"
                                    value={localConfig.s3?.retentionDays || 30}
                                    onChange={(e) => updateS3Config({ retentionDays: parseInt(e.target.value) || 30 })}
                                />
                            </div>
                            <div className="mt-4">
                                <AdminInput
                                    ...
                                    id="s3-endpoint"
                                    type="text"
                                    placeholder="https://s3.us-east-1.amazonaws.com (leave empty for AWS)"
                                    value={localConfig.s3?.endpoint || ''}
                                    onChange={(e) => updateS3Config({ endpoint: e.target.value })}
                                />
                                <p className="text-xs text-gray-500 mt-1">
                                    For S3-compatible services (MinIO, Backblaze B2, DigitalOcean Spaces, etc.)
                                </p>
                            </div>
                            <p className="text-xs text-gray-500 mt-4">
                                Create an IAM user with S3 permissions for the bucket. The backup service needs s3:PutObject, s3:GetObject, s3:ListBucket, and s3:DeleteObject permissions.
                            </p>
                        </div>
                    )}

                    {/* Google Drive Configuration */}
                    {localConfig.provider === 'gdrive' && (
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                            <h4 className="font-semibold text-white mb-4">Google Drive Configuration</h4>
                            <div className="space-y-4">
                                <AdminInput
                                    label="Client ID"
                                    id="gdrive-client-id"
                                    ...
                                    value={localConfig.gdrive?.clientId || ''}
                                    onChange={(e) => updateGDriveConfig({ clientId: e.target.value })}
                                />
                                <AdminInput
                                    label="Client Secret"
                                    ...
                                    value={localConfig.gdrive?.clientSecret || ''}
                                    onChange={(e) => updateGDriveConfig({ clientSecret: e.target.value })}
                                />
                                <AdminInput
                                    label="Refresh Token"
                                    ...
                                    value={localConfig.gdrive?.refreshToken || ''}
                                    onChange={(e) => updateGDriveConfig({ refreshToken: e.target.value })}
                                />
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <AdminInput
                                        ...
                                        value={localConfig.gdrive?.folderId || ''}
                                        onChange={(e) => updateGDriveConfig({ folderId: e.target.value })}
                                    />
                                    <AdminInput
                                        label="Retention (days)"
                                        id="gdrive-retention"
                                        ...
                                        min="1"
                                        max="365"
                                        value={localConfig.gdrive?.retentionDays || 30}
                                        onChange={(e) => updateGDriveConfig({ retentionDays: parseInt(e.target.value) || 30 })}
                                    />
                                </div>
                            </div>
                            <p className="text-xs text-gray-500 mt-4">
                                Create OAuth credentials in the Google Cloud Console. Enable the Google Drive API and generate a refresh token using the OAuth playground or a custom script.
                            </p>
                        </div>
                    )}
                    ...
                    {localConfig.provider && (
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                            <h4 className="font-semibold text-white mb-4">Test Connection</h4>
                            <p className="text-sm text-gray-400 mb-4">
                                Test your configuration before saving to ensure credentials and permissions are correct.
                            </p>
                            <AdminButton
                                variant="secondary"
                                ...
                                <div className="mt-4 space-y-2">
                                    ...
                                    {testResult.config && (
                                        <div className="text-xs text-gray-500 mb-3 font-mono">
                                            {testResult.config.endpoint && <div>Endpoint: {testResult.config.endpoint}</div>}
                                            {testResult.config.bucket && <div>Bucket: {testResult.config.bucket}</div>}
                                            {testResult.config.region && <div>Region: {testResult.config.region}</div>}
                                            {testResult.config.prefix && <div>Prefix: {testResult.config.prefix}</div>}
                                        </div>
                                    )}
                                    ...
                                    {testResult.tests.map((test, idx) => (
                                        ...
                                        >
                                            ...
                                            {test.message && (
                                                <p className="text-sm text-green-300 mt-1 ml-6">{test.message}</p>
                                            )}
                                            {test.error && (
                                                <p className="text-sm text-red-300 mt-1 ml-6">{test.error}</p>
                                            )}
                                            {test.suggestion && (
                                                <p className="text-sm text-yellow-300 mt-2 ml-6 italic">
                                                    💡 {test.suggestion}
                                                </p>
                                            )}
                                        </div>
                                    ...
                                    <div className={`mt-3 p-2 rounded text-center font-semibold ${
                                        ...
                                        {testResult.success ? 'All tests passed!' : 'Connection test failed'}
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    {/* Schedule Configuration */}
                    <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                        <h4 className="font-semibold text-white mb-4">Backup Schedule</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <AdminSelect
                                label="Frequency"
                                id="backup-frequency"
                                value={localConfig.schedule?.frequency || 'daily'}
                            ...
                            <AdminInput
                                ...
                                id="backup-time"
                                type="time"
                                value={localConfig.schedule?.timeOfDay || '03:00'}
                                onChange={(e) => updateSchedule({ timeOfDay: e.target.value })}
                            />
                        </div>
                        <p className="text-xs text-gray-500 mt-2">
                            Backups run at the scheduled time in your server's timezone.
                        </p>
                    </div>

                    {/* Options */}
                    <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                        <h4 className="font-semibold text-white mb-4">Options</h4>
                        <div className="space-y-4">
                            <AdminToggle
                                label="Keep Local Backups"
                                description="Keep encrypted backup files on the server after upload"
                                enabled={localConfig.keepLocalBackups || false}
                                onToggle={() => updateLocalConfig({ keepLocalBackups: !localConfig.keepLocalBackups })}
                            />
                            <AdminToggle
                                label="Notify on Success"
                                description="Send a webhook notification when backups complete successfully"
                                enabled={localConfig.notifyOnSuccess || false}
                                onToggle={() => updateLocalConfig({ notifyOnSuccess: !localConfig.notifyOnSuccess })}
                            />
                            <AdminToggle
                                label="Notify on Failure"
                                description="Send a webhook notification when backups fail"
                                enabled={localConfig.notifyOnFailure !== false}
                                onToggle={() => updateLocalConfig({ notifyOnFailure: !localConfig.notifyOnFailure })}
                            />
                        </div>
                    </div>

                    {/* Backup History */}
                    {status?.backupHistory && status.backupHistory.length > 0 && (
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                            <h4 className="font-semibold text-white mb-4">Recent Backups</h4>
                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                {status.backupHistory.map((backup, idx) => (
                                    <div key={idx} className={`flex items-center justify-between p-2 rounded ${backup.success ? 'bg-green-900/20' : 'bg-red-900/20'}`}>
                                        <div className="flex items-center gap-2">
                                            <span className={`w-2 h-2 rounded-full ${backup.success ? 'bg-green-400' : 'bg-red-400'}`}></span>
                                            <span className="text-sm text-white">{backup.filename || 'Unknown'}</span>
                                        </div>
                                        <div className="text-xs text-gray-400">
                                            {backup.timestamp ? new Date(backup.timestamp).toLocaleString() : 'Unknown time'}
                    ...
                    {/* Configuration Summary */}
                    {localConfig.enabled && localConfig.encryptionKey && localConfig.provider && (
                        <div className="bg-green-900/20 border border-green-700 rounded-lg p-4">
                            <h4 className="font-semibold text-green-300 mb-2">Configuration Ready</h4>
                            <div className="text-sm text-gray-300 space-y-1">
                                <p>Provider: {localConfig.provider === 's3' ? 'Amazon S3' : 'Google Drive'}</p>
                                <p>Schedule: {localConfig.schedule?.frequency === 'daily' ? 'Daily' : `Weekly on ${localConfig.schedule?.frequency}`} at {localConfig.schedule?.timeOfDay}</p>
                                <p>Retention: {localConfig.provider === 's3' ? localConfig.s3?.retentionDays : localConfig.gdrive?.retentionDays} days</p>
                            </div>
                        </div>
                    )}
...
// #region SmartThings Backup Settings

const SmartThingsBackupSettings = () => {
    const { addNotification } = useDashboard();
    const [config, setConfig] = useState<SmartThingsBackupConfig | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [isSaving, setIsSaving] = useState(false);
    const [isBackingUp, setIsBackingUp] = useState(false);
    const [isValidating, setIsValidating] = useState(false);
    const [backups, setBackups] = useState<SmartThingsBackupInfo[]>([]);
    const [latestBackup, setLatestBackup] = useState<any>(null);
    const [showPat, setShowPat] = useState(false);
    const [patInput, setPatInput] = useState('');
    const [patValidation, setPatValidation] = useState<SmartThingsPatValidation | null>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [configData, backupList, latest] = await Promise.all([
                    apiGetSmartThingsBackupConfig(),
                    apiListSmartThingsBackups(),
                    apiGetLatestSmartThingsBackup().catch(() => null)
                ]);
                setConfig(configData);
                ...
                console.error('Failed to fetch SmartThings backup config:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleValidatePat = async () => {
        if (!patInput.trim()) {
            addNotification('Please enter a Personal Access Token', 'error');
            return;
        }
        setIsValidating(true);
        setPatValidation(null);
        try {
            const result = await apiValidateSmartThingsPat(patInput);
            setPatValidation(result);
            if (result.valid) {
                addNotification(`PAT validated! Found ${result.locationCount} location(s)`, 'success');
            } else {
                addNotification(`PAT validation failed: ${result.error}`, 'error');
            }
        } catch (err: any) {
            setPatValidation({ valid: false, error: err.message });
            addNotification(`Validation error: ${err.message}`, 'error');
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (patInput.trim() && patValidation?.valid) {
                saveData.pat = patInput;
            }
            await apiSaveSmartThingsBackupConfig(saveData);
            const updatedConfig = await apiGetSmartThingsBackupConfig();
            setConfig(updatedConfig);
            setPatInput('');
            addNotification('SmartThings backup configuration saved!', 'success');
        } catch (err: any) {
            addNotification(`Failed to save: ${err.message}`, 'error');
        } finally {
            setIsSaving(false);
    ...
    const handleBackupNow = async () => {
        setIsBackingUp(true);
        try {
            const result = await apiTriggerSmartThingsBackup();
            if (result.success) {
                addNotification(`Backup completed! ${result.summary?.totalDevices} devices, ${result.summary?.totalScenes} scenes`, 'success');
                // Refresh backup list
                const [backupList, latest] = await Promise.all([
                    apiListSmartThingsBackups(),
                    apiGetLatestSmartThingsBackup().catch(() => null)
                ]);
                setBackups(backupList.backups || []);
                ...
                addNotification(`Backup failed: ${result.error || result.message}`, 'error');
            }
        } catch (err: any) {
            addNotification(`Backup error: ${err.message}`, 'error');
        } finally {
    ...
    const handleViewBackup = async (filename: string) => {
        try {
            const backup = await apiGetSmartThingsBackup(filename);
            setSelectedBackup(backup);
        } catch (err: any) {
            addNotification(`Failed to load backup: ${err.message}`, 'error');
        }
    };

    if (isLoading) {
        return <div className="p-8 text-center text-gray-400">Loading SmartThings backup configuration...</div>;
    }

    return (
        <>
            <AdminSection
                title="SmartThings Configuration Backup"
                description="Backup your SmartThings hub configuration (devices, scenes, rooms, automations) for disaster recovery. If your hub fails, this data helps you rebuild faster."
            >
                <div className="space-y-6">
                    {/* PAT Status */}
                    <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                        <h4 className="font-semibold text-white mb-3">Personal Access Token (PAT)</h4>
                        <p className="text-sm text-gray-400 mb-4">
                            A PAT is required to access your SmartThings configuration directly from Samsung's API.
                            Generate one at{' '}
                            <a href="https://account.smartthings.com/tokens" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">
                        ...
                        {config?.hasPat ? (
                            <div className="flex items-center gap-2 text-green-400 mb-4">
                                ...
                                <span>PAT configured and ready</span>
                            </div>
                        ) : (
                            <div className="flex items-center gap-2 text-yellow-400 mb-4">
                                <span className="w-3 h-3 rounded-full bg-yellow-400"></span>
                                <span>No PAT configured - backups disabled</span>
                            </div>
                        )}
                        ...
                        <div className="space-y-3">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    placeholder="Paste your new PAT here..."
                                ...
                                <AdminButton
                                    ...
                                    disabled={isValidating || !patInput.trim()}
                                    variant="secondary"
                                >
                            ...
                            {patValidation && (
                                <div className={`p-3 rounded ${patValidation.valid ? 'bg-green-900/20 border border-green-700' : 'bg-red-900/20 border border-red-700'}`}>
                                    {patValidation.valid ? (
                                        <div className="text-green-300">
                                            <p className="font-semibold">PAT Valid!</p>
                                            <p className="text-sm">Found {patValidation.locationCount} location(s):</p>
                                            <ul className="text-sm mt-1 ml-4 list-disc">
                                                {patValidation.locations?.map(loc => (
                                        ...
                                        <div className="text-red-300">
                                            ...
                                            <p className="text-sm">{patValidation.error}</p>
                                        </div>
                                    )}
                                </div>
                            )}
                    ...
                    {/* Enable/Disable */}
                    <AdminToggle
                        label="Enable Automated SmartThings Backups"
                        description="Automatically backup SmartThings configuration on a schedule"
                        enabled={localConfig.enabled}
                        onToggle={() => setLocalConfig(prev => ({ ...prev, enabled: !prev.enabled }))}
                    />

                    {/* Schedule */}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-300">Backup Schedule</label>
                        <select
                            value={localConfig.schedule}
                            onChange={(e) => setLocalConfig(prev => ({ ...prev, schedule: e.target.value as any }))}
                    ...
                    <div className="flex gap-3">
                        <AdminButton
                            onClick={handleBackupNow}
                            disabled={isBackingUp || !config?.hasPat}
                            variant="secondary"
                        >
                            {isBackingUp ? 'Backing up...' : 'Backup Now'}
                        </AdminButton>
                    ...
                    {config?.lastBackup && (
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                            <h4 className="font-semibold text-white mb-2">Last Backup</h4>
                    ...
                    {latestBackup && (
                        <div className="bg-blue-900/20 border border-blue-700 rounded-lg p-4">
                            <h4 className="font-semibold text-blue-300 mb-3">Latest Backup Summary</h4>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                                <div>
                                    <p className="text-gray-400">Locations</p>
                                    <p className="text-2xl font-bold text-white">{latestBackup.summary?.totalLocations || 0}</p>
                                </div>
                                <div>
                                    <p className="text-gray-400">Devices</p>
                                    <p className="text-2xl font-bold text-white">{latestBackup.summary?.totalDevices || 0}</p>
                                </div>
                                <div>
                                    <p className="text-gray-400">Rooms</p>
                                    <p className="text-2xl font-bold text-white">{latestBackup.summary?.totalRooms || 0}</p>
                                </div>
                                <div>
                                    <p className="text-gray-400">Scenes</p>
                                    <p className="text-2xl font-bold text-white">{latestBackup.summary?.totalScenes || 0}</p>
                                </div>
                                <div>
                                    <p className="text-gray-400">Automations</p>
                                    <p className="text-2xl font-bold text-white">{latestBackup.summary?.totalRules || 0}</p>
                                </div>
                                <div>
                                    <p className="text-gray-400">Apps</p>
                                    <p className="text-2xl font-bold text-white">{latestBackup.summary?.totalInstalledApps || 0}</p>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {backups.length > 0 && (
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                            <h4 className="font-semibold text-white mb-4">Backup History ({backups.length} backups)</h4>
                            <div className="space-y-2 max-h-64 overflow-y-auto">
                                {backups.map((backup, idx) => (
                                    <div
                                        key={idx}
                                        className="flex items-center justify-between p-2 rounded bg-gray-800 hover:bg-gray-700 cursor-pointer"
                                        onClick={() => handleViewBackup(backup.filename)}
                                    >
                                        <div className="flex items-center gap-2">
                                            <span className="w-2 h-2 rounded-full bg-green-400"></span>
                                            <span className="text-sm text-white">{backup.filename}</span>
                                        </div>
                                        <div className="text-xs text-gray-400">
                    ...
                    {/* Selected Backup Details Modal */}
                    {selectedBackup && (
                        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                            <div className="bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                                <div className="flex items-center justify-between p-4 border-b border-gray-700">
                                    <h3 className="text-lg font-bold text-white">
                                        Backup Details - {new Date(selectedBackup.timestamp).toLocaleString()}
                                    </h3>
                                    <button
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                    {selectedBackup.locations?.map((location: any) => (
                                        <div key={location.id} className="bg-gray-900 rounded-lg p-4">
                                            <h4 className="text-lg font-semibold text-white mb-3">{location.name}</h4>

                                            {/* Devices by Room */}
                                            <div className="mb-4">
                                                <h5 className="text-sm font-medium text-gray-300 mb-2">Devices by Room</h5>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                                    {Object.entries(location.devicesByRoom || {}).map(([roomId, data]: [string, any]) => (
                                                        <div key={roomId} className="bg-gray-800 rounded p-2">
                                                            <p className="font-medium text-white text-sm">{data.roomName} ({data.devices.length})</p>
                                                            <ul className="text-xs text-gray-400 mt-1">
                                                                {data.devices.slice(0, 5).map((d: any, i: number) => (
                                                                    <li key={i}>{d.label || d.name}</li>
                                                                ))}
                                                                {data.devices.length > 5 && (
                                                                    <li className="text-gray-500">... and {data.devices.length - 5} more</li>
                                                                )}
                                                            </ul>
                                                        </div>
                                                    ))}
                                            ...
                                            {location.scenes?.length > 0 && (
                                                <div className="mb-4">
                                                    <h5 className="text-sm font-medium text-gray-300 mb-2">Scenes ({location.scenes.length})</h5>
                                                    <div className="flex flex-wrap gap-2">
                                                        {location.scenes.map((scene: any, i: number) => (
                                            ...
                                            {location.rules?.length > 0 && (
                                                <div>
                                                    <h5 className="text-sm font-medium text-gray-300 mb-2">Automations ({location.rules.length})</h5>
                                                    <div className="flex flex-wrap gap-2">
                                                        {location.rules.map((rule: any, i: number) => (
                    ...
                    <div className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-4">
                        <h4 className="font-semibold text-yellow-300 mb-2">Recovery Guide</h4>
                        <p className="text-sm text-gray-300">
                            Each backup includes a recovery guide document that lists all devices, rooms, scenes, and automations.
                            While SmartThings requires manual device re-pairing after a hub failure, this guide dramatically
                            speeds up the process by documenting exactly what you had configured.
                        </p>
                        <p className="text-sm text-gray-400 mt-2">
                            Backups are stored in: <code className="text-blue-400">smartthings-backups/</code>
                        </p>
                    </div>
                </div>
            </AdminSection>
...
const Admin = () => {
    ...
    const tabs = [
        ...
        { id: 'cloud-backup', label: 'Cloud Backup', icon: IconCloud },
        { id: 'smartthings-backup', label: 'SmartThings Backup', icon: IconShieldAlert },
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 h-full">
            ...
            <div className="flex-1 min-w-0">
                ...
                {activeTab === 'smartthings-backup' && <SmartThingsBackupSettings />}
