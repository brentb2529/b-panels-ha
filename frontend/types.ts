...
export interface BatteryReportConfig {
  enabled: boolean;
  // Schedule configuration
  dayOfWeek: WeekDay; // Which day to send the report
  timeOfDay: string; // Time in HH:MM format (24-hour)
  timezone: string;
  // Email configuration
  smtpHost: string;
  smtpPort: number;
  smtpSecure: boolean; // true for SSL/TLS (port 465), false for STARTTLS (port 587)
  smtpUser: string;
  smtpPassword: string;
  fromEmail: string;
  fromName: string;
  recipientEmails: string[]; // List of email addresses to send the report to
  // Report configuration
  includeAllBatteries: boolean; // Include all devices with batteries, or only low ones
  lowBatteryThreshold: number; // Percentage threshold for "low" battery (default 20)
  criticalBatteryThreshold: number; // Percentage for "critical" battery (default 10)
  reportTitle: string; // Custom title for the email report
}

// Common battery types for device mappings
export type BatteryType = 'CR2032' | 'CR2450' | 'CR123A' | 'AA' | 'AAA' | 'CR2' | 'CR2477' | 'A23' | 'Rechargeable' | 'Other' | 'Unknown';

// Maps device IDs to their battery types
export interface BatteryTypeMappings {
  // Key is deviceId, value is battery type
  [deviceId: string]: BatteryType;
}

// Maps device IDs to the number of batteries they use
export interface BatteryQuantityMappings {
  [deviceId: string]: number;
}

// Default battery type mappings based on manufacturer + device name patterns
export interface BatteryTypeDefault {
  pattern: string; // Pattern to match against "manufacturer|deviceName"
  batteryType: BatteryType;
  description?: string; // Optional description for UI
}

// Battery level reading at a point in time
export interface BatteryReading {
  timestamp: string; // ISO 8601 format
  level: number; // Battery percentage 0-100
}

// Battery history for a single device
export interface DeviceBatteryHistory {
  deviceId: string;
  deviceName: string;
  readings: BatteryReading[];
  // Calculated prediction data (updated when readings are added)
  avgDailyDrain?: number; // Average % drain per day
  predictedCriticalDate?: string; // ISO 8601 date when expected to hit critical
  lastUpdated?: string; // ISO 8601 timestamp of last reading
}

// All battery history data
export interface BatteryHistoryData {
  devices: { [deviceId: string]: DeviceBatteryHistory };
  // Configuration
  maxReadingsPerDevice: number; // Limit stored readings (default 90 for ~3 months daily)
  recordingIntervalHours: number; // How often to record (default 24 = daily)
  lastRecordingTime?: string; // ISO 8601 timestamp
}
