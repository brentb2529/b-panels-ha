...
export const DashboardProvider = ({ children }: { children?: ReactNode }) => {
  ...
  const fetchDevicesFromServices = useCallback(async () => {
    ...
    if (useDemoMode) {
      ...
      const stConnection = connections.find(c => c.id === DeviceService.SmartThings && c.enabled);
      const sonosConnection = connections.find(c => c.id === DeviceService.Sonos && c.enabled);
      const haConnection = connections.find(c => c.id === DeviceService.HomeAssistant && c.enabled);
      const lutronConnection = connections.find(c => c.id === DeviceService.Lutron && c.enabled);
      ...
      if (stConnection) {
        try {
            const availableLocations = await smartThingsService.getLocations(stConnection);
            const locationMap = new Map(availableLocations.map(l => [l.locationId, l.name]));

            const locationsToFetch = (stConnection.selectedLocations && stConnection.selectedLocations.length > 0)
                ? stConnection.selectedLocations
                : availableLocations.map(l => l.locationId);

            if (locationsToFetch.length > 0) {
                const devicePromises = locationsToFetch.map(locationId =>
                    smartThingsService.getDevices(stConnection, locationId)
                );
                const rawDeviceArrays = await Promise.all(devicePromises);
                const rawDevices = rawDeviceArrays.flat();
                
                // Pre-process: Find which devices support battery based on capabilities
                const batteryDeviceIds = new Set<string>();
                rawDevices.forEach((d: any) => {
                    if (findCapability(d, 'battery')) {
                        batteryDeviceIds.add(d.deviceId);
                    }
                });

                const internalDevices = rawDevices
                    .map(d => mapStDeviceToInternalDevice(d, locationMap))
                    .filter((d): d is Device => d !== null);
                
                const initialStateSnapshot = await smartThingsService.getInitialState(stConnection);
                const stateMap = new Map(initialStateSnapshot.devices.map(d => [d.id, d.state]));
                
                // Queue for background refreshing of specific capabilities (batteries, shades, contacts, etc.)
                // Type allows us to handle different update logic
                const refreshQueue: { deviceId: string, locationId: string, task: 'battery' | 'shade' | 'contact' }[] = [];

                internalDevices.forEach(device => {
                    const relayState = stateMap.get(device.id);
                    
                    if (relayState && Object.keys(relayState).length > 0) {
                        const updates = parseRelayState(device.type, relayState);
                        if (updates.state !== undefined) {
                            device.state = updates.state;
                        }
                    ...
                    // Determine if device needs specific refresh
                    const isBatteryType = [
                    ...
                    const isShade = device.type === DeviceType.Shade;
                    const isContactSensor = device.type === DeviceType.ContactSensor;

                    if (device.locationId) {
                        // Battery Check
                        if (batteryDeviceIds.has(device.id) || isBatteryType) {
                            refreshQueue.push({ deviceId: device.id, locationId: device.locationId, task: 'battery' });
                        }

                        // Shade Level Check (Separate task to ensure we get the level capability specifically)
                        if (isShade) {
                            refreshQueue.push({ deviceId: device.id, locationId: device.locationId, task: 'shade' });
                        }

                        // Contact Sensor Check - needed for arming status hydration on load
                        if (isContactSensor) {
                            refreshQueue.push({ deviceId: device.id, locationId: device.locationId, task: 'contact' });
                        }
                    }
                });
                
                fetchedDevices.push(...internalDevices);

                // Background fetch for queued items (batteries, shades) to resolve potential stale/missing values
                if (refreshQueue.length > 0) {
                    (async () => {
                        for (const item of refreshQueue) {
                            // Use a delay to avoid hitting SmartThings API rate limits
                            await new Promise(r => setTimeout(r, 500));
                            try {
                                if (item.task === 'battery') {
                                    const capStatus = await smartThingsService.getDeviceCapabilityStatus(item.deviceId, item.locationId, 'main', 'battery', stConnection);
                                    if (capStatus && capStatus.battery && capStatus.battery.value != null) {
                                        const battVal = getNumericValue(capStatus.battery.value);
                                        if (battVal !== null) {
                                            setServiceDevices(current => produce(current, draft => {
                                                const dev = draft.find(d => d.id === item.deviceId);
                                                if (dev) dev.battery = battVal!;
                                            }));
                                        }
                                    }
                                } else if (item.task === 'shade') {
                                     // Shades rely on windowShadeLevel or windowShade capability
                                     let capStatus = await smartThingsService.getDeviceCapabilityStatus(item.deviceId, item.locationId, 'main', 'windowShadeLevel', stConnection);
                                     let level = getNumericValue(capStatus?.shadeLevel?.value) ?? getNumericValue(capStatus?.level?.value);
                                     
                                     if (level === null) {
                                         // Fallback to windowShade capability if level is missing
                                         capStatus = await smartThingsService.getDeviceCapabilityStatus(item.deviceId, item.locationId, 'main', 'windowShade', stConnection);
                                         const val = capStatus?.windowShade?.value;
                                         if (typeof val === 'string') {
                                             const s = val.toLowerCase();
                                             if (s.includes('open')) level = 100;
                                             else if (s.includes('close')) level = 0;
                                             else if (s.includes('partially')) level = 50;
                                         }
                                     }
                                     ...
                                     if (level !== null) {
                                         setServiceDevices(current => produce(current, draft => {
                                             const dev = draft.find(d => d.id === item.deviceId);
                                             if (dev) dev.state = level!;
                                         }));
                                     }
                                } else if (item.task === 'contact') {
                                    // Contact sensors - needed for arming status display on load
                                    const capStatus = await smartThingsService.getDeviceCapabilityStatus(item.deviceId, item.locationId, 'main', 'contactSensor', stConnection);
                                    if (capStatus && capStatus.contact && capStatus.contact.value != null) {
                                        // 'open' = true (sensor triggered), 'closed' = false (sensor secure)
                                        const isOpen = capStatus.contact.value === 'open';
                                        setServiceDevices(current => produce(current, draft => {
                                            const dev = draft.find(d => d.id === item.deviceId);
                                            if (dev) dev.state = isOpen;
                                        }));
                                    }
                                }
                            } catch (e) {
                                console.warn(`Failed background fetch for ${item.deviceId} (${item.task})`, e);
                            }
                        }
                    })();
                }

                const initialSthm = initialStateSnapshot.sthm.find(s => locationsToFetch.includes(s.locationId));
                if (initialSthm) {
                    const initialArmState = normalizeArmState(initialSthm.armState);
                    setSthmState({
                        locationId: initialSthm.locationId,
                        locationName: locationMap.get(initialSthm.locationId) || 'SmartThings',
                        armState: initialArmState,
                        securityState: 'OK',
                        violatingSensors: []
                    });
                    // Report initial STHM state to api-server for alarm detection
                    const apiArmState = initialArmState === 'armedStay' ? 'ARMED_STAY'
                        : initialArmState === 'armedAway' ? 'ARMED_AWAY' : 'DISARMED';
                    apiReportSTHMState(initialSthm.locationId, apiArmState)
                        .catch(e => console.warn('[STHM] Failed to report initial state:', e));
                } else {
                    const primaryLocationId = locationsToFetch[0];
                    setSthmState({
                        locationId: primaryLocationId,
                        locationName: locationMap.get(primaryLocationId) || 'SmartThings',
                        armState: 'disarmed',
                        securityState: null,
                        violatingSensors: []
                    });
                    // Report default disarmed state to api-server
                    apiReportSTHMState(primaryLocationId, 'DISARMED')
                        .catch(e => console.warn('[STHM] Failed to report initial state:', e));
                }
            } else {
                setSthmState(null);
             ...
             const errorMessage = (error as Error).message;
             console.error(`[SmartThings] Failed to fetch devices:`, errorMessage);
      ...
      if (sonosConnection) {
        try {
            const sonosPlayers = await sonosService.getPlayers(sonosConnection);
            fetchedDevices.push(...sonosPlayers);
        } catch (error) {
             const errorMessage = (error as Error).message;
             console.error(`[Sonos] Failed to fetch players:`, errorMessage);
             setServiceError(`[Sonos] ${errorMessage}`);
        }
      }

      if (haConnection) {
        try {
            const haEntities = await homeAssistantService.getDevices(haConnection);
            const haDevices = haEntities
                .map(mapHaEntityToInternalDevice)
                .filter((d): d is Device => d !== null);
            fetchedDevices.push(...haDevices);
        } catch (error) {
            const errorMessage = (error as Error).message;
            console.error(`[Home Assistant] Failed to fetch devices:`, errorMessage);
            setServiceError(`[Home Assistant] ${errorMessage}`);
      ...
      if (lutronConnection) {
        console.log('[Lutron] Connection found, fetching devices...');
        try {
            const lutronDevices = await apiGetLutronDevices();
            console.log('[Lutron] Fetched devices:', lutronDevices);
            fetchedDevices.push(...lutronDevices.map((d: any) => ({ ...d, service: DeviceService.Lutron })));
        } catch (error) {
             console.error(`[Lutron] Failed to fetch devices - Full error:`, error);
             const errorMessage = (error as Error).message || String(error);
             console.error(`[Lutron] Error message:`, errorMessage);
             setServiceError(`[Lutron] ${errorMessage}`);
        }
      } else {
        console.log('[Lutron] No enabled connection found. Connections:', connections.filter(c => c.id === DeviceService.Lutron));
      }

      if (etConnection) {
        try {
            const generators = await energyTrakService.getDevices(etConnection);
            fetchedDevices.push(...generators);
        } catch (error) {
            const errorMessage = (error as Error).message;
            console.error(`[EnergyTrak] Failed to fetch devices:`, errorMessage);
            setServiceError(`[EnergyTrak] ${errorMessage}`);
        }
      }

      if (whiskerConnection) {
        try {
            const robots = await litterRobotService.getDevices(whiskerConnection);
            fetchedDevices.push(...robots);
        } catch (error) {
            const errorMessage = (error as Error).message;
            console.error(`[LitterRobot] Failed to fetch robots:`, errorMessage);
            setServiceError(`[LitterRobot] ${errorMessage}`);
        }
      }

      if (haywardConnection) {
        try {
            const poolDevices = await haywardPoolService.getDevices(haywardConnection);
            fetchedDevices.push(...poolDevices);
        } catch (error) {
            const errorMessage = (error as Error).message;
            console.error(`[HaywardPool] Failed to fetch pool devices:`, errorMessage);
            setServiceError(`[HaywardPool] ${errorMessage}`);
        }
      }

      if (flairConnection) {
        try {
            const flairDevices = await flairService.getDevices(flairConnection);
            fetchedDevices.push(...flairDevices);
        } catch (error) {
            const errorMessage = (error as Error).message;
            console.error(`[Flair] Failed to fetch Flair devices:`, errorMessage);
            setServiceError(`[Flair] ${errorMessage}`);
        }
      }

      if (coolmasterConnection) {
        try {
            const coolmasterDevices = await coolMasterService.getDevices(coolmasterConnection);
            fetchedDevices.push(...coolmasterDevices);
        } catch (error) {
            const errorMessage = (error as Error).message;
            console.error(`[CoolMaster] Failed to fetch CoolMaster devices:`, errorMessage);
            setServiceError(`[CoolMaster] ${errorMessage}`);
        }
      }

      if (poolFloorConnection) {
        try {
            const floorDevices = await poolFloorService.getDevices(poolFloorConnection);
            fetchedDevices.push(...floorDevices);
        } catch (error) {
            const errorMessage = (error as Error).message;
            console.error(`[PoolFloor] Failed to fetch pool floor device:`, errorMessage);
            setServiceError(`[PoolFloor] ${errorMessage}`);
        }
      }
    }
